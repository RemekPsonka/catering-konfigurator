...
serve(async (req) => {
  ...
  try {
    const { inquiry_text, event_type_label, variants_summary, total_value, services_summary } = await req.json();
    ...
    if (variants_summary) contextParts.push(`Warianty:\n${variants_summary}`);
    if (total_value) contextParts.push(`Łączna wartość oferty: ${total_value} zł`);
    if (services_summary) contextParts.push(`Usługi dodatkowe:\n${services_summary}`);
        ...
        messages: [
          {
            role: "system",
            content:
              'Jesteś ekspertem od ofert cateringowych firmy "Catering Śląski". Napisz krótkie podsumowanie przygotowanej oferty (3-4 zdania). Opisz co zawiera oferta, ile wariantów, jakie wyróżniki. Odnieś się do zapytania klienta. Ton: profesjonalny, entuzjastyczny, po polsku. Nie używaj markdown. Zwracaj sam tekst.',
          },
